...
wait_for_instance() {
	local instance_id="$1"
	echo "Wating for instance $instance_id is now running."
	while true; do
...
create_ec2_instance() 
{
	...
	if [[ -z "$instance_id" ]];
	then
		echo "Failed to create EC2 instnce. " >&2
		exit 1
	fi
...
main() {
	...
	# Specify the parameters for creating the ec2 instance
	 ...
	 KEY_NAME="shellscripting_key"
	 SUBNET_ID="subnet-0cc1c695223df5b9e"
	 SECURITY_GROUP_IDS="sg-032e02dc66eed6171" # Add your security group IDs separated by space
